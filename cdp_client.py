...
class BaseCDP:
    ...
    def get_last_reply(self):
        ...
        (function() {{
            function extractTextWithLineBreaks(el) {{
                ...
                for (let child of el.childNodes) {{
                    if (child.nodeType === Node.TEXT_NODE) {{
                        ...
                        if (tag === 'br') {{
                            result += '\\n';
                        }} else if (['p','div','li','tr','h1','h2','h3','h4','h5','h6','section','header','footer'].includes(tag)) {{
            ...
            return extractTextWithLineBreaks(last).trim();
        }})()
        """
        return self.evaluate(js)
    ...
    def get_all_text(self):
        ...
        (function() {{
            ...
            if (all.length === 0) {{
                ...
                return chat ? extractTextWithLineBreaks(chat).trim() : '';
            }}
            return all.map(n => extractTextWithLineBreaks(n).trim()).join('\\n');
        }})()
        """
        return self.evaluate(js)
